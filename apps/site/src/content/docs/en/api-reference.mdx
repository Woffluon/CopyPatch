...
| `@copypatch/react` | `CopyPatchProvider`, `EditableText`, `useCopyPatch`, `useEditableText`, `useCopyPatchStore` | React client rendering and inline editing |
| `@copypatch/backend` | `createCopyPatchBackend`, `CopyPatchBackend` type | The embedded Web Request/Response runtime |
...
The store snapshot returned by `useCopyPatchStore` is deeply readonly and stable until a real state change occurs. Use `onEditorLoadError` on the provider to report a lazy editor-runtime load failure through host telemetry.
